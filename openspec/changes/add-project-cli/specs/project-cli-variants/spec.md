...
The system SHALL create new CUDA variants by copying a tracked template bundle into `variants/cuda/<variant-id>/` and registering it in `configs/variants.toml`.
...
- **AND** the CLI adds `moe-trial` to `configs/variants.toml`.
...
The system SHALL support copying the live managed CUDA bundle back into an existing registered variant.
