...
def _build_recorder_shell_command(manifest: TerminalRecordManifest) -> str:
    ...
    record_args = [
        "env",
        f"ASCIINEMA_CONFIG_HOME={run_root / 'asciinema-config'}",
        "pixi",
        "run",
        "asciinema",
        "rec",
    ...
    if manifest.mode == "active":
        record_args.append("--stdin")
    record_args.extend(
        [
            "-c",
            tmux_command,
            str(run_root / "session.cast"),
        ]
    )
    return f"{shlex.join(record_args)} > {shlex.quote(str(run_root / 'asciinema.log'))} 2>&1"
